...
Package scaffolding and documentation were made much easier thanks to the R packaging ecosystem, particularly `usethis`, `devtools`, `roxygen2`, `pkgdown`, `knitr`, and `rmarkdown`.
Interactive exploration is built with `shiny`, `plotly`, and `ggplot2`; data wrangling and utilities rely on `dplyr`, `tidyr`, `tibble`, and `scales`; theming uses `bslib`.
Simulation helpers use functions from base R’s `stats` package (e.g., lognormal and gamma draws).
